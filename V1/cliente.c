 ...
 */
 ...
 int main(int argc, char *argv[])
 {
  ...
  int sockfd;  
  struct hostent *he;
  struct sockaddr_in cliente; // informaci�n de la direcci�n de destino 
  ...
  if ((he=gethostbyname(argv[1])) == NULL) {  // obtener informaci�n de host servidor 
  ...
  cliente.sin_family = AF_INET;    // Ordenaci�n de bytes de la m�quina 
  cliente.sin_port = htons( atoi(argv[2]) );  // short, Ordenaci�n de bytes de la red 
  ...
  while(1){
    ...
    if (send(sockfd, comando, len_comando + 1, 0) == -1){
      fprintf(stderr, "Error al enviar el comando al servidor\n");
      exit(1);
    }else 
